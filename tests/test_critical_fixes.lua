...
describe('Phase 1: Critical Bug Fixes', function()
  ...
  describe('vtsls config', function()
    it('has conservative maxTsServerMemory (2048) in init.lua', function()
      ...
      assert.is_truthy(content:match('maxTsServerMemory%s*=%s*2048'),
        'Should have maxTsServerMemory = 2048 in init.lua')
    end)
  end)
  ...
  describe('mini.pairs', function()
    it('is configured with skip_next for common patterns in init.lua', function()
      ...
      assert.is_truthy(content:match('skip_next%s*='),
        'Should have skip_next configuration')
      assert.is_truthy(content:match('skip_ts%s*='),
        'Should have skip_ts configuration')
    end)
  ...
  describe('lspconfig API', function()
    it('uses vim.lsp.get_client_by_id or vim.lsp.get_clients({bufnr}) not deprecated buf_get_clients', function()
      ...
      assert.is_falsy(content:match('vim%.lsp%.buf_get_clients'),
        'Should not use deprecated vim.lsp.buf_get_clients')
      ...
      assert.is_truthy(has_modern_api,
        'Should use modern vim.lsp.get_client_by_id or vim.lsp.get_clients({bufnr=...})')
    end)
  end)
  ...
  describe('session', function()
    it('resolves symlinks before hashing cwd', function()
      ...
      assert.is_truthy(content:match('vim%.fn%.resolve'),
        'Should use vim.fn.resolve to handle symlinks in session path')
    end)
  end)
  ...
  describe('image.nvim TTY guard', function()
    it('has proper TTY guard (not just vim.fn.has(tty))', function()
      ...
      assert.is_truthy(has_gui_check and has_term_program,
        'Should have improved TTY check (gui_running and TERM_PROGRAM)')
      ...
      assert.is_falsy(has_tty,
        'Should not use deprecated has("tty") check in code')
    end)
  end)
  ...
  describe('diagnostics', function()
    it('virtual_text configured for current_line only in insert mode', function()
      ...
      assert.is_truthy(content:match('virtual_text%s*=%s*{%s*current_line%s*=%s*true'),
        'Should have virtual_text = { current_line = true }')
    end)
  end)
