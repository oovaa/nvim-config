...
describe('Phase 4: Polish', function()
  ...
  it('StartupTime shows lazy profile', function()
    ...
    assert.is_truthy(c:match('lazy%.stats%(%)'),
      ':StartupTime should print lazy.nvim stats()')
  end)

  it('treesitter skips scratch buffers (no TSInstall warning)', function()
    local c = read(init_path)
    assert.is_truthy(c:match('treesitter%-start.-buftype'),
      'treesitter FileType handler should skip non-file buffers')
  end)
end)
